...
- Impressão/PDF de estudo usa `PrintExportActions.astro` (ícones discretos Imprimir + Salvar como PDF em `SubjectActionBar.astro`, fora do grupo `#focus`) com `<dialog>` de checkboxes multisseleção (`src/lib/print-export.ts`: `conteudo`/`cheat-sheet`/`questoes`, default só a aba atual, gabarito só-letra default ligado); o bundle é montado em memória via `fetch` same-origin `force-cache` das rotas existentes + `window.print()` nativo, sem rota agregada, sem libs PDF, sem itens novos no offline e sem referências; limpeza em `afterprint`/`pagehide` + guarda de 60s.
