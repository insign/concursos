...
- `NextSubjectSuggestion.astro` é renderizado nas quatro rotas de assunto por `StudyLayout.astro` e `ReadingLayout.astro`; `src/lib/subject-suggestion.ts` agrupa assuntos pelo grupo editorial imediato (último `groupPath`/filhos diretos), escolhe o grupo pendente com a menor razão estudados explícitos/total, desempata pela pré-ordem determinística da árvore e indica o primeiro assunto pendente na ordem do grupo. Grupos nunca integram a identidade persistida, e o destino é a rota estática normal de conteúdo para suporte offline; SSR/sem JavaScript preserva um link real determinístico, o navegador o atualiza pelo documento e pelos eventos existentes de estudados, e a conclusão total produz uma mensagem acessível.
